...
void	save_col(t_mlx *mlx, t_col *col, int x, int y)
{
	t_col temp;
	...
	if (col->blue * 255.0f < 255.0f)
		...
		temp.blue = 255.0f;
	mlx->img.data[(x + y * WIN_X) * 4 + 0] = (unsigned char)temp.red;
	mlx->img.data[(x + y * WIN_X) * 4 + 1] = (unsigned char)temp.green;
	mlx->img.data[(x + y * WIN_X) * 4 + 2] = (unsigned char)temp.blue;
}

void	light_ray(t_env *mlx, t_col *col, float t, t_vec *dist)
{
	t_ray	light_ray;
	float	lambert;

	light_ray.start = mlx->obj.new_start;
	light_ray.dir = vector_scale((1 / t), dist);
	lambert = vector_dot(&light_ray.dir, &mlx->obj.n) * mlx->coef;
	col->r += lambert * mlx->obj.current_light.intensity.red
	* env->obj.current_mat.diffuse.red;
	col->g += lambert * env->obj.current_light.intensity.green
