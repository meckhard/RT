...
void	get_shape(const int fd, char *line, t_env *env)
{
	...
	if (ft_strcmp(line, "CONE") == 0)
		cone_list(fd, line, env);
	if (ft_strcmp(line, "PLANE") == 0)
		plane_list(fd, line, env);
}
...
void	get_camera(const int fd, char *line, t_env *env)
{
	...
	while (ft_strcmp(line, ".") != 0)
	{
		...
		if (ft_strcmp(temp[0], "R.X") == 0)
			env->r.start.x = ft_atof(temp[1]);
		if (ft_strcmp(temp[0], "R.Y") == 0)
			env->r.start.y = ft_atof(temp[1]);
...
void	get_cone(const int fd, char *line, t_cone *co)
{
	...
	while (ft_strcmp(line, "MATERIAL") != 0)
	{
		...
		temp = ft_strsplit(line, ' ');
		if (ft_strcmp(temp[0], "P.X") == 0)
			co->centre.x = ft_atof(temp[1]);
		if (ft_strcmp(temp[0], "P.Y") == 0)
			co->centre.y = ft_atof(temp[1]);
		if (ft_strcmp(temp[0], "P.Z") == 0)
			co->centre.z = ft_atof(temp[1]);
		if (ft_strcmp(temp[0], "RADIUS") == 0)
			co->radius = ft_atof(temp[1]);
		if (ft_strcmp(line, "MATERIAL") == 0)
		{
			get_material(fd, line, &co->m);
			break;
		}
	}
	free_map(&temp);
...
void	get_plane(const int fd, char *line, t_plane *p)
{
	...
	while (ft_strcmp(line, "MATERIAL") != 0)
	{
		...
		temp = ft_strsplit(line, ' ');
		if (ft_strcmp(temp[0], "P.X") == 0)
			p->centre.x = ft_atof(temp[1]);
		if (ft_strcmp(temp[0], "P.Y") == 0)
			p->centre.y = ft_atof(temp[1]);
		if (ft_strcmp(temp[0], "P.Z") == 0)
			p->centre.z = ft_atof(temp[1]);
		if (ft_strcmp(temp[0], "D.X") == 0)
			p->	normal.x = ft_atof(temp[1]);
		if (ft_strcmp(temp[0], "D.Y") == 0)
			p->normal.y = ft_atof(temp[1]);
		if (ft_strcmp(temp[0], "D.Z") == 0)
			p->normal.z = ft_atof(temp[1]);			
		if (ft_strcmp(line, "MATERIAL") == 0)
		{
			get_material(fd, line, &p->m);
			break;
		}		
	}
	free_map(&temp);
